...
export default function PublicSurvey() {
  ...
  const onSubmit = async (data: Record<string, unknown>) => {
    ...
    setSubmitting(false);
    if (error) { toast.error('Failed to submit ΓÇö please try again.'); return; }
  ...
  if (submitted) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 text-center">
        <div>
          <div className="text-5xl mb-6">≡ƒÖÅ</div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 bg-[radial-gradient(circle_at_30%_20%,rgba(59,130,246,.07),transparent_40%),var(--bg)]">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          {!project?.settings?.survey_welcome && (
            <p className="text-[var(--text2)] text-[13px]">Anonymous survey ΓÇö 100% confidential. No personal data collected.</p>
          )}
          {/* Language switcher */}
...
// ΓöÇΓöÇ Question Input ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function QuestionInput({ q, tOptions, register, watch, setValue }: QInputProps) {
  ...
  if (q.type === 'text') {
    return <Input placeholder="Your answerΓÇª" {...register(q.id)} onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }} />;
  }

  if (q.type === 'long_text') {
    return <Textarea placeholder="Your answerΓÇª" rows={4} {...register(q.id)} />;
  }
          ...
          return (
            ...
            >
              <div className={`w-4 h-4 rounded flex-shrink-0 border-2 flex items-center justify-center ${isSelected ? 'bg-[var(--accent)] border-[var(--accent)]' : 'border-[var(--border)]'}`}>
                {isSelected && <span className="text-white text-[10px] font-black">Γ£ô</span>}
              </div>
              {opt}
            </button>
          );
  ...
  return <Input placeholder="Your answerΓÇª" {...register(q.id)} />;
}
